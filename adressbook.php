...
//initialize session
session_start();
//check if user is logged in, if not go back to index
...
elseif(isset($_POST["add"])){
  ...
  $phone = $_POST["phone"];
  //prepare statement
  $stmt = $pdo->prepare("INSERT INTO adress(first_name,last_name,city,street,phone,user_id) VALUES(:first_name,:last_name,:city,:street,:phone,:user_id)");
...
elseif(isset($_POST["sample"])){
  //insertSample() needs an $user_id to assign the adress_id with the unique user_id
  $sqlite_create->insertSample($user_id);
}
?>
...
<body>
  <nav class="navbar navbar-light bg-light py-1">
    <div class="float-right">
      <a class="nav-link align-text-right" href="logout.php">Log Out</a>
    </div>
  </nav>
  <h1> Adressbook </h1>
...
<!-- Fill Table -->
      <?php
        ...
        $sql = "SELECT adress_id,first_name,last_name,city,street,phone,edit,user_id FROM adress WHERE user_id = " . $user_id . " ORDER BY " . $order_by . " COLLATE NOCASE " . $direction;
    ...
    <hr/>
    <form class="form-inline" method="POST" action="">
    <button type="submit" name="sample" class="btn btn-primary"> Insert Sample</button>
    </form>
    <footer style="position:relative;bottom:0;width:100%;height:50px;background-color: #ddd;">
        <div class="container">
